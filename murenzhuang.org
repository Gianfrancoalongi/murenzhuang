...
** Construction of problem system

Starting with the most basic program INPUT → OUTPUT, the possible continuation
in each step is one of the following

1. Remove a node from the system (any node not being INPUT or OUTPUT)
2. Add a random mutator node between two nodes (this will add a new path in the system)
3. Add a new output node from either the input node or a mutator node
